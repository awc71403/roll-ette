...
public abstract class TileBehavior : MonoBehaviour, IPointerClickHandler
{
    ...
    [SerializeField]
    GameObject TileHighlighterPrefab;
    ...
    [SerializeField]
    public GameObject moneyTileMarkerPrefab;
    public GameObject moneyTileMarker;

    public int isroulette = 0;
    ...
    public void HighlightCanMove(bool enemySelect=false)
    {
        ...
        tileHighlighterAnimator.SetBool("enemySelect", enemySelect);
        if (enemySelect)
        {
    ...
    public void HighlightCanAttack(bool enemySelect = false)
    {
        ...
        tileHighlighterAnimator.SetBool("selected", false);
        tileHighlighterAnimator.SetBool("enemySelect", false);
        highlighted = true;
        if (enemySelect)
        {
    ...
    void HighlightMoveableTiles(int moveEnergy, bool enemySelect=false)
    {
        ...
        foreach (Vector2 direction in directions)
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (otherTileUnit == null || otherTileUnit.GetComponent<Character>().player == selectedUnit.GetComponent<Character>().player)
                {
    ...
    public void OnPointerClick(PointerEventData data)
    {
        ...
        // Prevents you from clicking tiles while something else is going on
        if (LevelManager.singleton.testSingleton == true)
        {
            print("in allocation, ignore pointerclick");
            return;
        }

        if (LevelManager.actionInProcess)
        {
            return;
        }

        if (diceAttack.transform.GetComponent<DiceAttack>().IsRunning())
        {
            return;
        }
        ...
        if (selectionState == null)
        {
            ...
            if (myUnit != null)
            {
                ...
                if (LevelManager.currentPlayer.Equals(myUnit.GetComponent<Character>().GetPlayer()))
                {
            ...
            {
                // do nothing.
                print("you pressed an empty tile. congratulations");
            }
        }
        ...
        else
        {
            ...
            if (selectionState.Equals("move"))
            {
                ...
                else if (selectedTile.Equals(gameObject))
                {
                    ...
                    Deselect();
                    LevelManager.singleton.GetComponent<LevelManager>().ClearUI();
                }

                // and the unit's player is equal to to the current player...
                else if (myUnit != null && LevelManager.currentPlayer.Equals(myUnit.GetComponent<Character>().GetPlayer()))
                {
                    // select that unit/tile and highlight the tiles that the unit can move to (if it can move).
                    print("you selected a unit");
            ...
            else if (selectionState.Equals("attack"))
            {
                ...
                if (highlighted && myUnit != null && myUnit.GetComponent<Character>().GetPlayer() != selectedUnit.GetComponent<Character>().GetPlayer())
                {
                    ...
                    if (selectedUnit.GetComponent<Character>().GetName() == "Mage" && selectedUnit.GetComponent<Mage>().GetUseRoll())
                    {
                        print("GET HERE");
                        diceAttack.GetComponent<DiceAttack>().StartRolling(selectedUnit, myUnit);
                    }
                    else
                    {
                        // Dice stuff
                        if (!diceAttack.transform.GetComponent<DiceAttack>().IsRunning())
                        {
                            diceAttack.transform.GetComponent<DiceAttack>().StartRolling(selectedUnit, myUnit);
                        }
                    }
                    print("RESET");
                    SelectionStateToNull();
                ...
                {
                    // Dehighlight everything.
                    print("can't attack there, bitch");
                    SelectionStateToNull();
                }
            }
    ...
    public void SelectionStateToMove()
    {
        ...
        LevelManager.singleton.GetComponent<LevelManager>().ShowCharacterUI(selectedUnit);
    ...
    public void SelectionStateToUseAbil()
    {
        Deselect();

        // Switch selection state to move
        selectionState = "ability";
        ...
        // Use ability
        selectedUnit.GetComponent<Mage>().Ability();
    ...
    public static void AbilitySelection()
    {
        if (selectedUnit != null)
        {
            selectedTile.GetComponent<TileBehavior>().SelectionStateToUseAbil();
        }
    }
    #endregion
    ...
    {
        // Action in process!
        LevelManager.actionInProcess = true;
        ...
        // Action over!
        LevelManager.actionInProcess = false;
    }
    ...
    public void SetMoneyTile(bool moneyTile)
    {
        if (moneyTile && moneyTileMarker == null)
        {
            moneyTileMarker = Instantiate(moneyTileMarkerPrefab);
            moneyTileMarker.transform.SetParent(gameObject.transform, false);
        } else if (!moneyTile && moneyTileMarker != null)
        {
            Destroy(moneyTileMarker);
            moneyTileMarker = null;
        }
    }
}
